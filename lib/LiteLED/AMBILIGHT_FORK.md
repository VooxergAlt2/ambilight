...
- Stage 41 allocates two internal DMA bitstream buffers and pipelines CPU encoding of frame N+1 against the wire transfer of frame N;
- the PARLIO hardware queue depth is intentionally 1: Ambilight never builds a FIFO of stale LED frames, and only one DMA buffer can be hardware-owned at a time;
- the double-buffer ownership transition lives in `ll_parlio_pipeline.h` and is covered by native tests.
