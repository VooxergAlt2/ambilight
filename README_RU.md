...
- до **4 × 230 физических светодиодов**, максимум 920 адресов;
...
- управление питанием, яркостью, RGB и локальными эффектами из **Home Assistant** через WLED-compatible API;
...
- два app-slot по 7 MiB в 16 MiB flash для будущего OTA.
...
- короткий пропуск DDP-пакетов не создаёт искусственный чёрный кадр, последний полноценный кадр удерживается;
...
При выборе локального эффекта физический вывод временно переходит под управление ESP32. DDP при этом продолжает приниматься в фоне. Выбор `Ambilight` мгновенно возвращает управление последнему полному DDP-кадру.
...
- **Home** — состояние, яркость, DDP и ToF;
...
- **System** — Wi-Fi, backup/restore и factory reset.
...
Обычное обновление `firmware.bin` не стирает NVS. Кроме того, при загрузке прошивка не удаляет неизвестные schema-version, поэтому downgrade/recovery остаётся возможным.
...
- application image: **1 348 180 / 7 340 032 байт (18,4%)**.
